...
class DataIntepolator:
    ...
    Attributes:
        data (SurfaceData): Data obejct containing LAT and PtP
        weights (np.ndarray): Weights to interpolate point data to surface
    """
    ...
    def __init__(self, coords, segments):
        ...
        Args:
            coords (np.ndarray[N, 3]): Coords of the surface points
            segments (np.ndarray[N,]): Surface points labels (segment indexes)
        """
    ...
    def add_weights(self, coords):
        ...
        Args:
            coords (np.ndarray[N, 3]): Coords of the measuring electrodes
        """
    ...
    def interpolate(self, electrodes_data):
        ...
        Args:
            electrodes_data (SurfaceData): Data object containing LAT and PtP
                                           at measuring electrodes

        Returns:
            SurfaceData: Data object with interpolated LAT and PtP
        """
        out = self.data.copy()
    ...
    def compute_weights(self, surface_points, electrode_points, r=30):
        ...
        Args:
            surface_points (np.ndarray): Unknown point coords (surface)
            electrode_points (np.ndarray): Known point coords (electrodes)
            r (float, optional): All point within radius will be used for
                                 interpolation

        Returns:
            np.ndarray: Weights for interpolation
        """
